...
func NewClusterAppsCommand(resolveApps func(flags action.AppsFlags) ([]types.AppId, error)) *cobra.Command {
	f := action.AppsFlags{
...
This currently follows the same behavior as hydra local apps: only app-id
resolution (including --exclude-app), no template rendering, no cluster access.

When no app-id pattern is provided, Hydra uses '**' (all apps).`,
		Example: `  # List all apps for a cluster prefix
  hydra gitops apps in-cluster.** --hydra-context /path/to/context

  # List all apps (default pattern '**')
  hydra gitops apps --hydra-context /path/to/context

  # Exclude one app from the result set
  hydra gitops apps in-cluster.** --exclude-app in-cluster.apps.argocd`,
